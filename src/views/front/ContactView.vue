  ...
  <v-container>
    <v-row class="mt-7">
      ...
      <v-col cols="12" class="text-center">
        <v-form v-model="valid" @submit.prevent="submitMessage">
          <v-textarea v-model="message" name="回饋訊息" label="回饋訊息" counter maxlength="500" placeholder="輸入訊息"
            variant="outlined" :rules="rules.message">
          </v-textarea>
          <div class="contact_btn_group">
            <v-btn variant="text" color="error" type="reset">重填</v-btn>
            <v-btn variant="text" color="success" type="submit">送出</v-btn>
          </div>
        </v-form>
...
const rules = reactive({
  message: [
    v => !!v || '必填',
    v => (v.length > 0 && v.length <= 500) || '回饋訊息為 1 到 500 字'
  ]
})
</script>